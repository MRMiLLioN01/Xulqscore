-- Fix: PL/pgSQL variables qualified as submit_assessment.<var> resolved as a table
-- reference at runtime ("missing FROM-clause entry"). All locals are now v_ prefixed
-- so no qualification is needed and nothing collides with column names.
